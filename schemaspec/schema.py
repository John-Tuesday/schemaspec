...
Represent key/value schema with `SchemaItem` and non-inline tables with `SchemaTable`.
Top-Level tables are represented by `Schema`
"""
...
class SchemaItem[T]:
    ...
    def is_valid(self, value: T) -> bool:
        """True if `value` is valid to at least one adapter in `possible_values`."""
...
class SchemaTable[T](adapters.TypeAdapter[T]):
    """Table of key-value options and/or subtables."""
    ...
    def __init__(
        ...
        :param `make_cls`: No parameter callable which returns a new default
            instance of `T`.
    ...
    def add_item[
        ...
        """Add a new `SchemaItem()`.

        Forwards all parameters to `SchemaItem()`

        :param `name`: Sets `SchemaItem.short_name` in the new item.

        :return: The new instance of `SchemaItem()`.
        """
        item = SchemaItem(
            short_name=name,
    ...
    def add_subtable[
        ...
        """Create a `SchemaTable` within this table and return it.

        :param `make_cls`: Forwarded to `SchemaTable()`.
        :param `name`: Name of the table; full name is automatically generated by
            joining the full name of `this` and `name` with a dot `'.'`.
        :param `description`: Forwarded to `SchemaTable()`.

        :return: The created subtable.
        """
        table = SchemaTable(
            make_cls=make_cls,
    ...
    def parse_data(
        ...
        the attribute in namespace will be set to `SchemaItem.default_value`.

        :param `data`: Map-like object representing basic parsed output, e.g. the output
            of `tomllib.load()`.
        :param `namespace`: Namespace-like object whose attributes will be modified.
            If `namespace` is `None`, use the output of `make_cls()` as supplied in
            the constructor `SchemaTable()`.
        :param `error_mode`: Choose what happens when incoming data cannot be
            converted to its corresponding type.

        :return: Populated `namespace` or output of `make_cls()`.
    ...
    def format_export(
        ...
        """Create valid schema output such that it could recreate `namespace`.
        ...
        :return: A valid schema string representing `namespace`, including only children
            found in `keys` or all children if `keys` is empty.
    ...
    @override
    def export(self, value: T) -> str | None:
        """Export `value` as an inline-table."""
        l = [
...
class Schema[T](SchemaTable[T]):
    ...
    def load_toml(
        ...
        :param `filepath`: Path to a TOML configuration file.
        :param `namespace`: Namespace forwarded to `parse_data()`.

        :return: Output of `parse_data()`.
        """
